...
- The chunk must have `schemaVersion: "native-scoring-batch-v7.0.0"`, `type: "context"`, 1–5 jobs, one non-empty batch ID, unique job IDs, and a versioned `contextProfile`.
...
- Return one `ruleProvenance` entry for every resulting bullet in identical order. Echo `ruleText` exactly. An unchanged existing rule must use an empty `sourceDecisionIds` array. Every new or materially reworded rule must cite at least one exact assigned feedback job ID that directly supports that rule; never attribute every job in the batch to every rule.
...
```json
{
  "contextUpdate": {
    ...
    "ruleProvenance": [
      {
        "ruleText": "concise negative rule",
        "sourceDecisionIds": ["exact assigned feedback job ID supporting this new rule"]
      }
    ],
    "processedFeedback": [
      {
...
Before responding, verify exact keys, exact ordered feedback IDs and timestamps, exact ordered rule provenance, negative-only rules, no Markdown fence, and valid bare JSON.
