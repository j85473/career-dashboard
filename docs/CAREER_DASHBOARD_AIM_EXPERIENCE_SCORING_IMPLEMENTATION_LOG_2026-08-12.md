...
## Unified 50-job manual-scoring batches — 2026-08-21

- Increased the Aim and Experience v2 export/result membership contracts from 30 to 50 jobs, including ordinal bounds, API/UI defaults, server-side batch validation, and Aim runner authority metadata. Model-call concurrency remains unchanged, so larger batches increase queued work without increasing simultaneous model pressure.
- Increased the atomic serializable scoring-import transaction timeout from 30 to 60 seconds. Preview, explicit approval, exact membership, protected lifecycle behavior, safe-failure release, and all-or-nothing apply semantics remain unchanged.
- Because the v2 schema hashes and Aim runner-protocol hash are scoring input authorities, deploying this change intentionally makes any older outstanding, unimported v2 batch stale. Operators must import or explicitly release existing batches before deployment; completed batches and accepted score events are not reinterpreted or invalidated.

## Whole-queue scoring runs with recoverable children — 2026-08-31

- Replaced the routine 50-job Dashboard export with one stage-specific whole-queue run export. The parent run snapshots and reserves all currently eligible jobs, then binds them into exact 40-job child batches. Aim and Experience retain independent one-active-run constraints.
- Added a database-free local run controller with durable recovery state, two concurrent child batches, and one shared four-model-call ceiling. A restart validates and skips completed children; a completed rerun returns the same byte-identical upload artifact instead of generating a divergent result.
- Consolidated child outputs into one run result and one Desktop upload file. Dashboard preview verifies the parent hash, child order, exact manifests, every existing child result contract, current source/input authority, and protected lifecycle state.
- Kept the proven child import transaction as the write boundary. Apply is atomic per child, so a later failure does not roll back or replay earlier accepted children. Explicit release frees only remaining leases and preserves accepted scores. Superseded children are shown as a blocked parent run.
- Preserved the Experience hard-mismatch safety boundary: any `hard_requirement_mismatch` stops artifact publication until the main Codex agent audits every mismatch against the exact JD and complete Core Evidence and finalizes an exact review-bound result.
- Added a 2,000-job/64 MiB pre-lease safety ceiling, parent/child readiness auditing, focused recovery and semantic-review tests, and retained the legacy single-batch contracts for old outstanding exports and one-job Aim retries. No existing score is invalidated or requeued by this change.
