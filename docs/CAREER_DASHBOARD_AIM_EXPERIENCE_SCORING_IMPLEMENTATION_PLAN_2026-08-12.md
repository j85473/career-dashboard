...
- The current designated Dashboard resume is the repository-owned `data/resumes/JosephLamb_Resume.docx`, SHA-256 `9ad3e6c9db671d455aab2d903d3d662e81d385883a436663b597286850c77640`; its stable machine filename is independent of the human-facing Desktop export title.
- The deployed Pi exports only the hash-verified repository artifact. It does not read Joseph's Mac Desktop at runtime and has no Desktop or database fallback.
- At release time, the production exporter verifies the repository artifact against the recorded hash. A Desktop quick-access copy may be checked byte-for-byte as a publication receipt, but it is not scoring authority. No process may fall back to the first database resume, a historical resume, or a same-named file with different bytes.
