...
ARCHIVE = Path(os.environ['M70_ARCHIVE'])
OUTPUT = ARCHIVE.parent / 'row-comparison.json'
MANIFEST = ARCHIVE.parent / 'archive-rows.json'
DATABASE = os.environ['M70_DATABASE']
EXPECTED_SHA = os.environ['M70_ARCHIVE_SHA256']
...
def main():
    ...
    if not re.fullmatch(r'career_[a-z0-9_]+', DATABASE) or not re.fullmatch(r'[a-f0-9]{64}', EXPECTED_SHA):
        raise RuntimeError('Unexpected database or checksum')
    mode = sys.argv[1:]
