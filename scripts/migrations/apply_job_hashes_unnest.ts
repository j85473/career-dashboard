...
async function run() {
  ...
  // CRITICAL: Force Postgres to use index scan instead of sequential scan!
  ...
  console.log('Updating hashes for remaining jobs using fast UNNEST...');
  let updateCount = 0;
  ...
  try {
    for (let i = 0; i < updatesArray.length; i += 5000) {
      ...
      const hashes = chunk.map(c => c[1]);
      
      const query = `
        UPDATE "Job"
        SET fingerprint = data.fingerprint
        FROM (
          SELECT unnest($1::text[]) AS id, unnest($2::text[]) AS fingerprint
        ) AS data
        WHERE "Job".id = data.id;
      `;
      await client.query(query, [ids, hashes]);
