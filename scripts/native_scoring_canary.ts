...
assert.equal(NATIVE_SCORING_EXPECTED_MODEL, 'gemini-3.6-flash-high');
...
assert.match(standardEvaluator, /Immutable Standard Evaluator V7\.0\.0/);
assert.match(standardEvaluator, /native-scoring-batch-v7\.0\.0/);
assert.match(contextEvaluator, /Immutable Negative-Only Context Evaluator V6\.7\.1/);
assert.match(contextEvaluator, /native-scoring-batch-v7\.0\.0/);
assert.match(manager, /Immutable V6\.7\.0 Scoring Manager/);
assert.match(standardEvaluator, /`criterionAssessments` \(array of 1–32 objects\)/);
assert.match(standardEvaluator, /never add, omit, merge, split, paraphrase, or reorder criteria/);
...
assert.match(standardEvaluator, /Silence means `cannot_evaluate`/);
assert.match(standardEvaluator, /evidence inventory is positive-only/);
assert.match(standardEvaluator, /scan every narrative string literally/);
assert.match(standardEvaluator, /invalid silence-based negative claim is invalid/);
assert.match(standardEvaluator, /Application code owns Experience weighting, caps, labels, pass\/fail, salary, and travel/);
assert.match(standardEvaluator, /Software-license management and product licensing knowledge remain ordinary functional criteria/);
...
assert.doesNotMatch(standardEvaluator, /`compensation` \(string or null\)/);
assert.match(standardEvaluator, /Do not extract or return compensation/);
assert.match(directImport, /compensation: evaluation\.score\.compensation/);
assert.match(directImport, /refreshEvidenceGapReport\(prisma\)/);
