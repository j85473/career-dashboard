export function passesPreFilter(job: { title: string, description: string, location: string, url: string, company: string }): { passes: boolean, reason: string } {
  ...
  if (/\b(warehouse|driver|delivery|barista|bartender|server|waiter|waitress|janitor|cleaner|housekeeper|housekeeping|laborer|teller|dispatcher|student worker|food service|cook|chef|hostess|busser|dishwasher|security guard|valet|baggage handler|factory|assembly|production worker|technician|hospitality|hotel|motel|resort|casino|bellhop|concierge|guest services)\b/i.test(titleLower) && !/\b(manager|director|vp|head|lead|supervisor)\b/.test(titleLower)) {
  ...
  if (/\b(banking center|teller|personal financial representative|financial services representative|financial advisor|financial planner|private wealth|private wealth management|SBA underwriter|underwriting professional|proprietary trader|WM affluent banker|claims adjuster|claims adjustor|claims examiner|claims specialist|claims supervisor|claims representative|workers.compensation claims|liability claims|enrollment processor)\b/i.test(titleLower)) {
