...
const SOURCE_BOILERPLATE_HEADING = /^(?:who\s+are\s+we\??|grow\s+with\s+us|culture(?:\s*&\s*total\s+rewards|\s+and\s+total\s+rewards)?|cool\s+things\s+to\s+know|the\s+important\s+stuff|what\s+success\s+looks\s+like|pay\s+transparency)\s*:?$/i;
// Postal addresses and contact blocks reach criteria when a posting concatenates
// its footer into the qualifications section without separators.
// No `\b` anchors: the artifact is precisely that the footer was concatenated
// without separators, so "Equipment12325 River Road" and "55056Contact:" have
// no word boundary where one is expected.
const CONTACT_BLOCK = /contact:\s*[a-z]+\s+[a-z]+|\d{3,6}\s+[a-z]+\s+(?:road|street|avenue|drive|lane|boulevard|court)\b/i;
...
const COMPENSATION_DISCLAIMER = /\b(?:base\s+pay[^.]{0,80}?will\s+(?:vary|depend|be\s+based)|(?:starting|base|actual)\s+(?:base\s+)?salary\s+will\s+be\s+determined|pay\s+(?:offered|range)[^.]{0,80}?will\s+(?:vary|depend|be\s+based)|compensation\s+(?:at\s+.{1,80}\s+)?varies\s+depending\s+on|compensation\s+(?:offer|offered|for\s+this\s+role).{0,100}(?:based\s+on|may\s+vary|determined\s+by)|compensation\s+will\s+be\s+determined\s+commensurate\s+with|compensation,?\s+.{1,80}\s+takes\s+into\s+consideration\s+factors|(?:specific\s+office\s+)?location,?\s+role,?\s+skill\s+set,?\s+and\s+level\s+of\s+experience|experience,?\s+skills,?\s+job\s+duties,?\s+and\s+business\s+need)\b/i;
const HIRING_AND_CAREER_BOILERPLATE = /\b(?:accessible\s+and\s+inclusive\s+hiring\s+experience|fully\s+demonstrate\s+their\s+skills|opportunities\s+to\s+keep\s+skills\s+relevant\s+through\s+certifications|values-based\s+culture,?\s+upward\s+mobility,?\s+and\s+professional\s+development|deepen\s+connections,?\s+maintain\s+a\s+strong\s+community,?\s+and\s+do\s+their\s+best\s+work|equivalent\s+amounts\s+of\s+relevant\s+experience\s+may\s+be\s+considered\s+in\s+lieu)\b/i;
...
function compact(value: string | null | undefined): string {
  // `&amp;` decodes last so a double-encoded `&amp;lt;` resolves to the literal
  // text `&lt;` in a single pass, exactly as a browser would, instead of being
  // unescaped twice into a live `<`.
...
function normalizedLines(description: string): string[] {
  // Closing tags accept trailing whitespace and stray attributes because browsers
  // do: `</script foo="bar">` still ends a script. `\b` keeps `</scripty>` out.
  const withoutExecutableMarkup = description.replace(/<script\b[^>]*>[\s\S]*?<\/script\b[^>]*>/gi, ' ').replace(/<style\b[^>]*>[\s\S]*?<\/style\b[^>]*>/gi, ' ').replace(/<(?:br|\/p|\/li|\/div|\/h[1-6]|\/section)\s*\/?>/gi, '\n').replace(/<li\b[^>]*>/gi, '\n- ').replace(/<[^>]+>/g, ' ').replace(/\*\*([^*\n]{2,100})\*\*/g, '\n$1\n').replace(/\s+\+\s+(?=[A-Z0-9])/g, '\n+ ').replace(/\s*•\s*/g, '\n• ').replace(/\r\n?/g, '\n').replace(/\u0000/g, ' ');
...
function headingSection(line: string): PacketSection | 'excluded' | null {
  ...
  if (/^(?:preferred|preferred experience|preferred qualifications?|preferred skills?|desired qualifications?|nice to have|bonus points?|accelerators?|ideal candidate|what will our ideal candidate have\??)$/i.test(normalized)) return 'PREFERRED EXPERIENCE';
...
export function assertNativeScoringEvaluationPacket(value: string): void {
  ...
  const criterionStart = value.indexOf('REQUIRED EXPERIENCE\n');
  const criterionEnd = value.indexOf('ROLE-DEFINING QUALIFICATIONS\n');
  const criterionText = value.slice(criterionStart, criterionEnd);
  if (/^\s*-\s*(?:preferred|required)\s+skills?\s*$/im.test(criterionText)) {
    throw new Error('evaluation packet contains a qualification-heading placeholder');
  }
  if (/\b(?:base\s+pay|base\s+salary|salary|compensation)\b.{0,180}\b(?:vary|varies|fluctuat(?:e|es)|factor|depending|job-related|candidate(?:'|’)s\s+location)\b/i.test(criterionText)) {
    throw new Error('evaluation packet contains pay boilerplate in Experience');
  }
