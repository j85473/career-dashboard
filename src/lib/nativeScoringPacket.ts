...
const COMPENSATION_DISCLAIMER = /\b(?:base\s+pay\s+will\s+vary\s+based\s+on|compensation\s+(?:at\s+.{1,80}\s+)?varies\s+depending\s+on|compensation\s+(?:offer|offered|for\s+this\s+role).{0,100}(?:based\s+on|may\s+vary|determined\s+by)|compensation\s+will\s+be\s+determined\s+commensurate\s+with|compensation,?\s+.{1,80}\s+takes\s+into\s+consideration\s+factors|(?:specific\s+office\s+)?location,?\s+role,?\s+skill\s+set,?\s+and\s+level\s+of\s+experience|experience,?\s+skills,?\s+job\s+duties,?\s+and\s+business\s+need)\b/i;
