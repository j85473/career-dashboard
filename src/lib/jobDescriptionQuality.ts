export function looksLikeInvalidJobDescription(value: string): boolean {
  ...
  if (!text) return true;
  const terminalPage = /\b(?:404|page not found|page does not exist|page you (?:are|were) looking for (?:does not|doesn't) exist|job (?:is )?no longer (?:available|active|posted)|(?<!until (?:this |that |the )?)position (?:has been|is) filled|posting (?:has been|is) closed)\b/.test(text);
