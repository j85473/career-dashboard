...
test('V6.10.1 standard evaluator is bound to the canonical Workday resume', async () => {
...
test('V6.10.1 evaluator embeds the exact trusted evidence mirror', () => {
  const evaluator = fs.readFileSync(evaluatorPath, 'utf8');
...
test('V6.10.1 evaluator states the required-domain cross-field invariants without contradiction', () => {
  const evaluator = fs.readFileSync(evaluatorPath, 'utf8');
...
test('V6.10.1 evaluator makes binary credentials fail closed without misclassifying software licenses', () => {
  const evaluator = fs.readFileSync(evaluatorPath, 'utf8');

  assert.match(evaluator, /Never mark a binary credential `adjacent`/);
  assert.match(evaluator, /Field work, travel, territory ownership, employment history/);
  assert.match(evaluator, /mark any such supplied requirement candidate `unsupported`/);
  assert.match(evaluator, /combines supported experience with an unsupported binary credential, the whole candidate is `unsupported`/);
